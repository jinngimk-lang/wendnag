...
> SHA-256 is recorded for integrity verification. The current ChatGPT GitHub write connector accepts UTF-8 text but has no local-binary file parameter, so these binary outputs are delivered in-chat and mirrored in the downloadable project package. Do not treat the binaries as committed to GitHub until an actual Git binary upload is performed.
